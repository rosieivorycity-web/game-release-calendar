...
def make_candidates(releases, games, pmap, types, statuses):
    ...
    for gid, rows in bygame.items():
        ...
        # IGDB's Main Game type is ID 0. Some API serializers may omit
        # a zero-valued reference field, so a missing game_type is treated
        # as Main Game (0), not as an invalid type.
        raw_type = g.get("game_type", 0)
        if raw_type is None:
            raw_type = 0

        t = types.get(int(raw_type), "")
...
def main():
    ...
    print("Fetched game_type counts (missing treated as Main Game / 0):")
